...
class ListViewTest(TestCase):
    ...
    def test_displays_only_items_for_that_list(self):
        ...
        Item.objects.create(text='Incorect item 1', list=incorrect_list)
        Item.objects.create(text='Emkurekt item 2', list=incorrect_list)

        response = self.client.get(f'/lists/{test_list.id}')

        self.assertContains(response, 'Item1')
        self.assertContains(response, 'Item42')
        self.assertNotContains(response, 'Incorect item 1')
        self.assertNotContains(response, 'Emkurekt item 2')
